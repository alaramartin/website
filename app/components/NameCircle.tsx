...
export default function NameCircle() {
  ...
  const rotate = useTransform(scrollYProgress, [ADJ_END, ROTATE_END], [0, 360]);
  ...
  useMotionValueEvent(rotate, "change", (v) => setRotationDeg(normalizeDeg(v)));
  ...
  useLayoutEffect(() => {
    const measure = () => {
      ...
      const top = (el: Element) => el.getBoundingClientRect().top - origin.top;
    ...
    <div ref={stageRef} className="relative h-svh w-full">
      ...
      >
        ...
        >
          ...
          <span ref={restBLRef} style={{ display: "inline-block", width: 0 }} />
        </p>
        <p
          className="whitespace-nowrap leading-none"
          ...
          >
            ...
            >
              {rays.map(({ title, angle }) => {
                ...
                const currentRadius = isName ? fullRadius : fullRadius * adj;

                const baseOpacity = distance <= activeWindow ? 1 : 0.7;
                ...
                return (
                  ...
                  >
                    ...
                    >
                      {isName ? (
                        <>
                          <span ref={rayLaraRef}>LARA&nbsp;</span>
                          <span ref={rayMartinRef}>MARTIN</span>
                        </>
                      ) : (
                        title
                      )}
